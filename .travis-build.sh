...
$builddep -q -y python-rasterstats.spec

if [[ $image =~ ^fedora: || $image =~ ^centos: ]]
then
    mkdir -p ~/rpmbuild/{BUILD,BUILDROOT,RPMS,SOURCES,SPECS,SRPMS}
    cp python-rasterstats.spec ~/rpmbuild/SPECS/
    spectool -g -R -S ~/rpmbuild/SPECS/python-rasterstats.spec
    set +x
    rpmbuild -ba ~/rpmbuild/SPECS/python-rasterstats.spec 2>&1 | pv -q -L 3k
else
